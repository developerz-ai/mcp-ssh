...
| `src/oauth/` | minimal OAuth 2.1 server: discovery metadata, dynamic client registration, authorize + token with PKCE, bearer validation; tokens persisted in SQLite (`src/db.rs`) so logins survive a restart; `sweep_expired_access`/`sweep_expired_refresh` bulk-delete dead rows (count only, never a token value) for the reaper to call |
