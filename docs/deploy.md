...
| `MCP_SSH_ALLOWED_HOSTS` | `localhost,127.0.0.1` (loopback binds only) | hostnames accepted in `Host` — **set to your public hostname**. **Required** when `MCP_SSH_BIND` is non-loopback (e.g. `0.0.0.0`): startup fails without it |
