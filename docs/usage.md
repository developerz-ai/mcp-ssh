...
| Still running after the window | A **job id** (e.g., `cargo-build-23:30`) — monitor it with `job(action="poll")` |
...
→ "job cargo-build-23:30 still running after the inline window. Poll it with job(action=\"poll\", id=\"cargo-build-23:30\")."

job(action="poll", id="cargo-build-23:30")
→ {"status":"running"}
  Compiling mcp-ssh v0.1.0
  ...200 lines...
  [lines ..200 of 540; next_cursor=200]

job(action="poll", id="cargo-build-23:30", cursor=200)
→ ...next page...

job(action="poll", id="cargo-build-23:30", cursor=540)
...
```
job(action="poll", id="cargo-build-23:30")                       # first page
job(action="poll", id="cargo-build-23:30", cursor=200)           # next page
job(action="poll", id="cargo-build-23:30", cursor=400, limit=500)
job(action="list")                                                # all jobs + status
job(action="kill", id="cargo-build-23:30")                       # kill a running job
```
